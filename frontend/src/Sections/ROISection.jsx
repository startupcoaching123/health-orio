...
import { TrendingUp, ArrowRight, Clock, PieChart, Building, Wallet, ArrowUpRight } from 'lucide-react';
...
const ROISection = ({ theme }) => {
  ...
  const [compRef, compVisible] = useOnScreen({ threshold: 0.2 });
  const [drvRef, drvVisible] = useOnScreen({ threshold: 0.2 });
  ...
  const drivers = [
    ...
    { label: "Better Payer Mix", icon: Wallet },
  ];

  return (
    <section id="roi-section" className={`w-full ${bgClass} ${textClass} font-sans transition-colors duration-700 border-t border-white/5 overflow-x-hidden`}>

      {/* --- COMPARISON --- */}
      <div ref={compRef} className="w-full max-w-6xl mx-auto px-6 py-24">

        <div className={`text-center mb-12 transition-all duration-1000 ${compVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        ...
        <div className="relative grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-10 items-center">
          ...
          <div className={`hidden lg:flex absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-20 w-12 h-12 rounded-full bg-emerald-500 text-white items-center justify-center shadow-xl transition-all duration-1000 delay-500 ${compVisible ? 'scale-100 opacity-100' : 'scale-0 opacity-0'}`}>
            <ArrowRight size={20} />
          </div>

          {/* Before */}
          <div className={`relative p-5 md:p-8 rounded-[2rem] border-2 border-dashed transition-all duration-1000
          ...
          {/* After */}
          <div className={`relative p-5 md:p-8 rounded-[2rem] border-2 shadow-2xl overflow-hidden transition-all duration-1000 delay-200
          ...
          `}>
            ...
            <div className="flex justify-between items-start mb-6">
              ...
              <div className="px-2 py-0.5 rounded-full bg-emerald-500 text-white text-[10px] font-bold shadow-md">Active Optimization</div>
            </div>
        ...
        <div className={`mt-12 mx-auto max-w-lg p-6 rounded-2xl text-center border transition-all duration-1000 delay-500
          ...
          ${compVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}
        `}>
      ...
      {/* --- DRIVERS --- */}
      <div ref={drvRef} className="w-full max-w-7xl mx-auto px-6 pb-24">
        <div className={`mb-10 text-center transition-opacity duration-1000 ${drvVisible ? 'opacity-100' : 'opacity-0'}`}>
          ...
          {drivers.map((item, index) => (
            <div
              ...
              className={`flex flex-col items-center justify-center text-center p-6 rounded-2xl border transition-all duration-500 hover:-translate-y-2
                ${cardBg}
                ${drvVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}
              `}
